...
async function enrichSingleItem(item, context, dayItems = [], index = 0) {
  ...
  const resolvedItem = await resolveSpecificItem(item, context, dayItems, index)
  if (shouldSkipLookup(resolvedItem)) return resolvedItem

  const cityQuery = buildCityQuery(context.city, context.country)
  const textMatch = resolvedItem.place_id
    ? { place_id: resolvedItem.place_id, geometry: { location: { lat: resolvedItem.lat, lng: resolvedItem.lng } } }
    : await findPlaceByText({
...
export async function enrichLooseItems(items = [], context = {}) {
  return Promise.all(items.map((item, index) => enrichSingleItem(item, context, items, index)))
}
...
export async function enrichItineraryDays(itineraryDays = [], context = {}) {
  ...
  for (const day of completedDays) {
    const enrichedItems = await enrichLooseItems(day.items ?? [], context)
    const route = await build_day_route({ items: enrichedItems })

    let routeIndex = 0
    const itemsWithRoute = enrichedItems.map((item) => {
      if (item.type === 'transport' || item.type === 'note' || item.lat == null || item.lng == null) {
        return item
      }
      if (routeIndex === 0) {
        routeIndex += 1
      ...
      const segment = route.route_segments[routeIndex - 1] ?? null
      routeIndex += 1
      if (!segment) return item
      return {
        ...item,
        route_from_previous: segment,
      }
    })
