...
async function fetchWithTimeout(url, init = {}, timeoutMs = ROUTE_FETCH_TIMEOUT_MS) {
...
export async function check_transport({ from_lat, from_lng, to_lat, to_lng, mode = 'driving' }) {
  try {
    ...
    console.error('Google Directions fallback to OSRM:', error?.message || error)
  }
  ...
  try {
    ...
    console.error('OSRM directions failed:', error?.message || error)
  }
  ...
  return {
    ...
    mode,
    steps: [],
    note: 'Route timed out or not found.',
  }
}
