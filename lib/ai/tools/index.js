...
export const TOOL_DEFINITIONS = [
  ...
  },
  {
    ...
    function: {
      name: 'search_places',
      description: 'Search Google Maps for specific physical places, restaurants, hotels, or attractions. For food, call this only when the user explicitly asks for named restaurants, cafes, eateries, places to eat, nearby restaurants, or a specific physical food spot.',
      parameters: {
        type: 'object',
        properties: {
          query: { type: 'string', description: 'The search query (e.g., "halal food near KLCC", "luxury hotels in Tokyo", "tourist attractions")' },
          lat: { type: 'number', description: 'Latitude to bias the search (usually the destination city center)' },
          lng: { type: 'number', description: 'Longitude to bias the search (usually the destination city center)' },
        },
        required: ['query'],
      },
    },
                ...
                items: {
                  ...
                  items: {
                    ...
                    properties: {
                      ...
                      type: { type: 'string', enum: ['attraction', 'restaurant', 'food_recommendation', 'transport', 'note'], description: 'Type of place' },
                      requires_ticket: { type: 'boolean', description: 'True if this attraction requires purchasing a ticket or paying an entry fee' },
                      price_estimate: { type: 'string', description: 'Estimated cost of the item, activity, or meal (e.g. "RM 50", "$15", or "Free"). Required for restaurants and paid attractions.' },
          ...
          updates: {
            ...
            items: {
              ...
              properties: {
                ...
                type: { type: 'string', enum: ['attraction', 'restaurant', 'food_recommendation', 'transport', 'note'], description: 'Type of place (only for add/update)' },
                ...
                price_estimate: { type: 'string', description: 'Estimated cost of the item (e.g. "RM 50" or "Free"). Required for paid items (only for add/update).' },
                ...
                new_name: { type: 'string', description: 'The new name for the place (only for update action if renaming)' },
...
const TOOL_STATUS = {
  ...
  search_places: (args) => `🔍 Searching for "${args.query}"...`,
