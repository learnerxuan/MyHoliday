...
- LOGISTICS (MANDATORY): Always use \`type: 'note'\` for logistics like "Arrival", "Departure", "Check-in", and "Check-out". These are not attractions and must not be pinned on the map.
...
- IMPORTANT: Distinction between 'restaurant' and 'food_recommendation' tags (MANDATORY):
    * 'restaurant' -> ONLY for specific, named establishments (e.g. "Ichiran Ramen", "L'Antica Pizzeria da Michele"). USE THIS for real places you want to recommend.
    * 'food_recommendation' -> For all generic suggestions, local guidance, or broad descriptions (e.g. "dinner at a local Izakaya", "authentic street food").
    * MIX IT UP: You should provide a high-quality mix of both specific restaurants (hidden gems/famous spots) and generic food recommendations (for flexibility). Do not rely solely on one type.
    * Any activity named "Lunch at a...", "Dinner at a...", or "Breakfast at a..." MUST be tagged as food_recommendation because it is a placeholder suggestion.
    * If the name includes words like "local", "authentic", "nearby", "shop", "stall", or "vendor", YOU MUST use 'food_recommendation'.
    * Generic items will not be pinned on the map to prevent irrelevant global results.
